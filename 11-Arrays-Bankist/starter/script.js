...
btnLoan.addEventListener('click', event => {
  ...
  const loanCheck = currentAccount.movements.some(mov => mov >= amount * 0.1);
  if (amount > 0 && loanCheck) {
    currentAccount.movements.push(amount);
    updateUI(currentAccount);
  }
...
//   const correctJulia = dogsJulia.slice(1, 3);
...
// const movementsUSD = movements.map(el => Math.trunc(el * eurToUsd));
...
//   (el, idx) =>
//     `${idx + 1} You ${el > 0 ? 'deposited' : 'withdrew'} ${Math.abs(el)}`
// );
...
//   return acc + cur; // return is necessary!!!!!!!!!!!!
// }, 0);
...
//   const humanAges = ages.map(el => (el <= 2 ? el * 2 : el * 4 + 16));
//   console.log(humanAges);
//   const above18 = humanAges.filter(el => el >= 18);
//   console.log(above18);
//   // const average = Math.trunc(
//   //   above18.reduce((acc, cur) => acc + cur, 0) / above18.length
//   // );
//   const average = Math.trunc(
//     above18.reduce((acc, cur, i, arr) => acc + cur / arr.length, 0)
//   );
//   console.log(average);
// };
...
// console.log(accounts[3].movements.every(deposit));
...
// // Previous examples using reduce
...
const { ownersEatTooMuch, ownersEatTooLittle } = dogs.reduce(
  (acc, cur) => {
    acc[`ownersEatToo${cur.curFood > cur.recFood ? 'Much' : 'Little'}`].push(
      ...cur.owners
    );
    return acc;
  },
